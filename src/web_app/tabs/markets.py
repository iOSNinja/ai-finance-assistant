...
def _render_indices() -> None:
    ...
    for col, (name, quote) in zip(cols, overview["indices"].items(), strict=False):
        with col:
            _render_quote_card(name, quote)
    if overview.get("cache_hit"):
        st.caption(
            f"🕒 Cached — refreshed at most {overview['indices'].get('S&P 500', {}).get('cache_hit') and '30 min' or 'recently'} ago."
        )
...
def _render_stock_lookup() -> None:
    ...
    c5.metric(
        "52-week high",
        f"${quote['fifty_two_week_high']:,.2f}" if quote["fifty_two_week_high"] else "—",
    )
    c6.metric(
        "52-week low",
        f"${quote['fifty_two_week_low']:,.2f}" if quote["fifty_two_week_low"] else "—",
    )
    ...
    st.caption(f"{hist['data_points']} data points · {hist['start_date']} → {hist['end_date']}")
...
def render() -> None:
    ...
    st.caption(
        f"Live market data via yfinance. Last loaded: {datetime.now().strftime('%H:%M:%S')}."
    )
...
def _fetch_ticker_news(ticker: str, limit: int = 5) -> list[dict]:
    ...
    for index, item in enumerate(raw, start=1):
        ...
        if "content" in item and isinstance(item["content"], dict):
            logger.debug(
                "Detected wrapped yfinance news format",
                extra={"ticker": ticker_upper, "item_index": index},
            )
            ...
            logger.debug(
                "Detected flat yfinance news format",
                extra={"ticker": ticker_upper, "item_index": index},
            )
...
def _render_ticker_news(ticker: str) -> None:
    ...
    logger.info(
        "Rendering news articles", extra={"ticker": ticker_upper, "article_count": len(articles)}
    )
