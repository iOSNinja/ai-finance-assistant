...
class OrchestratorDecision(BaseModel):
    ...
    agents: list[
        Literal[
            "qa_agent", "tax_agent", "goal_agent", "portfolio_agent", "market_agent", "news_agent"
        ]
    ] = Field(
...
def orchestrator_node(
    ...
    decision: OrchestratorDecision = routing_llm.invoke(
        [SystemMessage(content=ORCHESTRATOR_PROMPT), *history, HumanMessage(content=query)],
        config={
    ...
    logger.info(
        "Routing decision made", extra={"agents": decision.agents, "reasoning": decision.reasoning}
    )
