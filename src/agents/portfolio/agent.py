...
def portfolio_agent_node(state: FinnieState) -> dict:
    ...
    if loop_cnt >= MAX_AGENT_ITERATIONS:
        ...
        return {"portfolio_messages": [AIMessage(content=fallback)], "portfolio_response": fallback}
    ...
    try:
        ...
        logger.error(
            "Portfolio agent LLM call failed",
    ...
    if has_tools:
        logger.info(
            "Portfolio agent requested tool calls",
        ...
        logger.info(
            "Portfolio agent: produced final answer",
            extra={"response_len": len(update["portfolio_response"])},
        )
...
def should_continue_portfolio(
    state: FinnieState,
) -> Literal["portfolio_tools_node", "synthesizer_node"]:
