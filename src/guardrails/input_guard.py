...
def check_input(query: str) -> InputGuardResult:
    ...
    if len(query) < MIN_QUERY_LEN:
        logger.info("Input blocked", extra={"guard_type": "length", "category": "too_short"})
        return InputGuardResult(False, "Query too short", "too_short")
    if len(query) > MAX_QUERY_LEN:
        logger.info(
            "Input blocked",
            extra={"guard_type": "length", "category": "too_long", "query_len": len(query)},
        )
    ...
    for pat in INJECTION_PATTERNS:
        ...
        if match:
            ...
            return InputGuardResult(False, "Regex injection match", "prompt_injection")
    ...
    try:
        ...
        if result.flagged:
            ...
            return InputGuardResult(False, f"Moderation: {','.join(flagged)}", "moderation_flagged")
    except Exception as e:
        logger.warning(
            "Moderation API error — failing open",
    ...
    if injected:
        logger.info(
            "Input blocked",
            extra={"guard_type": "llm_classifier", "category": "llm_classifier_flagged"},
        )
        return InputGuardResult(False, "LLM classifier flagged injection", "llm_classifier_flagged")
    ...
    combined_redactions = [{**r, "via": "regex"} for r in regex_redactions] + [
    ...
    return InputGuardResult(
        ...
        category="ok",
        cleaned_query=text,
        input_redactions=combined_redactions,
    )
