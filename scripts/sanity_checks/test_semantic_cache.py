...
print(f"  Q1 'What is an ETF?'                          -> {'HIT' if r1 else 'MISS'}  (expect MISS)")
...
print(f"  Q5 unrelated 'AAPL price'                     -> {'HIT' if r5 else 'MISS'}  (expect MISS)")
...
print(f"  Same query after TTL window                   -> {'HIT' if r6 else 'MISS'}  (expect MISS)")
...
print(f"  (should be the last 3 — the first 'ETF' query evicted)")
